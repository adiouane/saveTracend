...
export default function AdminsMembers({ user } : { user: any}) {
 ...
 // list all members in the channel
  ...
  const hanndleMembers = (member :any) => {
    setSetting(!Setting);
  }
  ...
  const kickMember = (member : string, channelId: string) => {
    ...
    socket.on("kickMember", (data: any) => {
      alert("Member "+member + " kicked successfully");
      setSetting(!Setting);
    });
  }
  ...
  const BanMember = (member : string, channelId: string) => {
    ...
    socket.on("BanMember", (data: any) => {
      alert("Member "+member + " banned successfully");
      setSetting(!Setting);
    });
  }
          ...
          <div key={member?.username}  className="ml-8 w-56">
...

            {
              Setting && (
                ...
                // kick user
                // ban user
                <div className="absolute top-0 left-[465px] right-0 bottom-0 bg-slate-800 w-[666px] h-[300px] rounded-lg flex flex-col items-center justify-center">                 
                  ...
                  onClick={hanndleMembers}>close</button>
                </div>
              )
            }
          </div>
