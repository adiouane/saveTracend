...
export default function ChatContent({ user }: { user: any }) {
  ...
  const { channel, setChannel } = useChannleTypeStore(); // FIXME: THIS IS ERROR BECUSE CHANNEL IS GLOBAL STATE AND WILL CHANGE BEETWEEN DIFFERENT USERS
  ...
  useEffect(() => {
      ...
      if (username || messageInput || channel) {
        ...
        socket.on("listChannelMessages", (data) => {
          ...
          if (data.msg.length === 0) {
            ...
            alert("messages is in channel :" + channel);
            // if (data.msg[0].channel === channel) {
            //   setSenderMessages([]);
    ...
    } else 
    {
      ...
      if (!isblockUser){
        ...
        socket.on("listDirectMessages", (data) => {
          ...
          if (Array.isArray(data.msg) && data.msg.length === 0) { // to handdle an error ecured in browser console
    ...
    if(Array.isArray(messages)) { // to handdle an error ecured in browser console
    ...
    // };
  }, [username, channel, isDirectMessage, messages, isblockUser]);
        ...
        <div className=" p-14 flex-1 overflow-auto">
          {
            <div>
              {arrayMessages.map((message: any, index: number) => (
                <div key={message.id} className="flex flex-col mb-4 text-sm">
